...
- “查询番剧”通过 `?page=search` 进入独立页面，始终从 `allAnime` 搜索全部已收录作品，不能改为只搜索当前周、当前季度或日历中可见的节目。查询须支持中文、日文标题的规范化匹配，并保留无结果提示与浏览器前进／后退支持。
...
- 节目详情使用原生 `<dialog>`。打开元素需要在关闭后恢复焦点，所有可点击节目卡应保留正确的按钮语义和 `aria` 标签。详情弹窗内含追番开关、观看进度和逐集已看按钮；逐集按钮的 `(episodeStart, episode)` 单元必须来自 `episodeViewUnitsForAnime`，与周历展开的已看键保持一致（先行/连播多集合并为一个范围键），不要在 JSX 里自行假设每集都是单集键。
...
- “播出表”“我的番剧”和“查询番剧”分别通过默认页、`?page=mine`、`?page=search` 切换。修改路由状态时保留浏览器后退/前进支持，不要改成仅有 React 内部状态的页面切换。
