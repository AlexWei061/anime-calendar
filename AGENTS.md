...
- 全站是亮色（樱花粉彩）与暗色（霓虹夜色）双主题：颜色只能通过 `app/globals.css` `:root` 中的设计 token
  ...
  新增或调整颜色时必须同时维护两套主题取值并使用 token，不要在组件规则里写硬编码色值。
...
- 已看标记同样按账号邮箱隔离并永久保存在 D1；单条记录由 `animeId`、`episodeStart` 和 `episode` 唯一标识。API 必须先认证，再通过 `validateEpisodeView` 校验；只接受 `episodeViewUnitsForAnime` 生成的规范单集，不能接受浏览器提供的用户身份或任意集数范围。旧版本遗留的合法连播范围记录应在读取时展开并迁移为单集记录。
