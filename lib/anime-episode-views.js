...
export function episodeViewUnitsForRange({ episodeStart, episode }) {
  const units = [];
  for (let currentEpisode = episodeStart; currentEpisode <= episode; currentEpisode += 1) {
    units.push({ episodeStart: currentEpisode, episode: currentEpisode });
  }
  return units;
}

// 观看状态始终按单集保存；连播只影响日历中的排期卡片，不合并详情里的集数。
export function episodeViewUnitsForAnime(record) {
  return episodeViewUnitsForRange({ episodeStart: 1, episode: record.episodeCount });
}

function legacyEpisodeViewUnitsForAnime(record) {
  const units = [];
...
export function isLegacyEpisodeViewForAnime(value, record) {
  if (!value || typeof value !== "object") return false;
  const { episodeStart, episode } = value;
  return (
    Number.isInteger(episodeStart) &&
    Number.isInteger(episode) &&
    episodeStart < episode &&
    legacyEpisodeViewUnitsForAnime(record).some(
      (unit) => unit.episodeStart === episodeStart && unit.episode === episode,
    )
  );
}

export function isEpisodeViewWatched(watchedEpisodes, watchedEpisode) {
  return episodeViewUnitsForRange(watchedEpisode).every(({ episode }) =>
    watchedEpisodes.some(
      (candidate) =>
        candidate.animeId === watchedEpisode.animeId &&
        candidate.episodeStart <= episode &&
...
export function updateEpisodeViews(watchedEpisodes, watchedEpisode, watched) {
  const normalizedEpisodes = watchedEpisodes.flatMap((candidate) =>
    episodeViewUnitsForRange(candidate).map((unit) => ({ animeId: candidate.animeId, ...unit })),
  );
  const episodeViews = episodeViewUnitsForRange(watchedEpisode).map((unit) => ({
    animeId: watchedEpisode.animeId,
    ...unit,
  }));
  const keys = new Set(episodeViews.map(episodeViewKey));
  if (watched) {
    return normalizedEpisodes.concat(
      episodeViews.filter(
        (episodeView) =>
          !normalizedEpisodes.some((candidate) => episodeViewKey(candidate) === episodeViewKey(episodeView)),
      ),
    );
  }
  return normalizedEpisodes.filter((candidate) => !keys.has(episodeViewKey(candidate)));
}
...
export function validateEpisodeViewBatch(value, animeById, maxSize = 25) {
  if (!Array.isArray(value)) {
    throw new TypeError("watchedEpisodes must be an array");
  }
  if (value.length === 0) {
    throw new RangeError("watchedEpisodes must not be empty");
  }
  if (value.length > maxSize) {
    throw new RangeError(`watchedEpisodes must contain at most ${maxSize} items`);
  }
...
export function filterKnownEpisodeViews(value, animeById) {
  ...
  for (const candidate of value) {
    try {
      const anime = animeById.get(candidate?.animeId);
      const units = isLegacyEpisodeViewForAnime(candidate, anime)
        ? episodeViewUnitsForRange(candidate).map((unit) => ({ animeId: candidate.animeId, ...unit }))
        : [validateEpisodeView(candidate, animeById)];
      for (const watchedEpisode of units) {
        const key = episodeViewKey(watchedEpisode);
        if (!seen.has(key)) {
          seen.add(key);
          watchedEpisodes.push(watchedEpisode);
