...
export function dateOnlyEventsForWeek(records, weekStart) {
  ...
  return records
    .filter(
      ({ premiereDateBeijing, premiereKind, scheduleWeekday, beijingTime }) =>
        premiereKind === "network" && !scheduleWeekday && !beijingTime && dates.has(premiereDateBeijing),
    )
    .map((record) => ({ ...record, date: record.premiereDateBeijing }))
