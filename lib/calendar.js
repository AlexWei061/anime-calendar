...
export const TIMELINE_EVENT_DURATION_MINUTES = 30;
...
function eventsFromEpisodeSchedules(record, dates) {
  const events = [];

  for (const schedule of record.episodeSchedules) {
    if (schedule.intervalDays === 0) {
      const layout = layoutBroadcast(schedule.broadcastDateBeijing, schedule.beijingTime);
      if (dates.has(layout.date)) {
        events.push({ ...record, ...layout, episodeStart: schedule.episodeStart, episode: schedule.episodeEnd });
      }
      continue;
    }

    for (let episode = schedule.episodeStart; episode <= schedule.episodeEnd; episode += 1) {
      const layout = layoutBroadcast(
        addDays(schedule.broadcastDateBeijing, (episode - schedule.episodeStart) * schedule.intervalDays),
        schedule.beijingTime,
      );
      if (dates.has(layout.date)) events.push({ ...record, ...layout, episodeStart: episode, episode });
    }
  }
...
export function eventsForWeek(records, weekStart) {
  ...
  for (const record of records) {
    if (record.episodeSchedules?.length) {
      events.push(...eventsFromEpisodeSchedules(record, dates));
      continue;
    }
    if (!record.scheduleWeekday || !record.beijingTime) continue;

    const premiereEpisodeCount = record.premiereEpisodeCount ?? 1;
    const firstScheduledEpisode = premiereEpisodeCount + (record.premiereKind === "network" ? 1 : 0);
    const scheduledWeekday = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].indexOf(
      record.scheduleWeekday,
    );
    const premiereWeekday = parseIsoDate(record.premiereDateBeijing).getUTCDay();
    const firstRegularBroadcastDate =
      record.regularBroadcastStartDateBeijing ??
      addDays(record.premiereDateBeijing, (scheduledWeekday - premiereWeekday + 7) % 7 || 7);
    for (let episode = firstScheduledEpisode; episode <= record.episodeCount; episode += 1) {
      const broadcastDate =
        episode === premiereEpisodeCount
          ? record.premiereDateBeijing
          : addDays(firstRegularBroadcastDate, (episode - premiereEpisodeCount - 1) * 7);
...
export function dateOnlyEventsForWeek(records, weekStart) {
  const dates = new Set(weekDays(weekStart));

  return records
    .filter(
      ({ premiereDateBeijing, premiereKind }) => premiereKind === "network" && dates.has(premiereDateBeijing),
    )
    .map((record) => ({
      ...record,
      date: record.premiereDateBeijing,
      episodeStart: 1,
      episode: record.premiereEpisodeCount ?? 1,
    }))
