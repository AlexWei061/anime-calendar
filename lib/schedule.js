...
export function toBeijingAiring({
  premiereDateJst,
  jstTime,
  premiereDateBeijing,
  scheduleWeekday,
  ...
  if (beijingTime !== null && beijingTime !== undefined) {
    if (!/^(\d+):(\d{2})$/.test(beijingTime)) {
      throw new RangeError(`Invalid Beijing time: ${beijingTime}`);
    }
...
export function groupByBeijingWeekday(records) {
  ...
  for (const record of records) {
    ...
    if (record.beijingTime === null || record.beijingTime === undefined) {
