  ...
  return createServer(async (request, response) => {
    try {
      ...
      const allowed = url.pathname === "/api/watched" ? "GET, PUT"
        : ["/health", "/api/anime", "/api/summary"].includes(url.pathname) ? "GET" : null;
      ...
      if (url.pathname === "/api/summary") {
        // TODO：从 watched 计算已看过的番剧部数和已看单集数，然后返回 200。
        // 同一番剧至少一集已看才计一部；重复 PUT 不应重复计数。
        return sendJson(response, 501, { error: "请完成 GET /api/summary" });
      }
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (!Number.isInteger(port) || port < 1024 || port > 65535) {
    ...
    server.listen(port, "127.0.0.1", () => {
      console.log(`练习 04：http://127.0.0.1:${port}（Ctrl+C 停止）`);
    });
  }
}
