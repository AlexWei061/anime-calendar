...
async function generateSeason(config, aniListIndex, year, usedIds, syoboiSchedules) {
  ...
  for (const [index, card] of cards.entries()) {
    ...
    anime.push({
      ...
      coverUrl: await downloadCover(card.coverUrl, year, config.month, index),
    });
  }
...
async function main() {
  ...
  const [aniListCatalogs, syoboiSchedules] = await Promise.all([
  ...
  for (const config of yucSeasons) {
    catalogs.push(await generateSeason(config, aniListIndex, year, usedIds, syoboiSchedules));
  }
