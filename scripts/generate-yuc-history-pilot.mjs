...
async function downloadCover(coverUrl, logicalCoverUrl) {
  if (coverSpriteFor(logicalCoverUrl)) return logicalCoverUrl;
  const prefix = logicalCoverUrl.slice(0, -extname(logicalCoverUrl).length);

  for (const localCoverUrl of LOCAL_COVER_EXTENSIONS.map((extension) => `${prefix}${extension}`)) {
    try {
  ...
  const localCoverUrl = `${prefix}${coverExtension(response, coverUrl)}`;
  await writeFile(new URL(`../public${localCoverUrl}`, import.meta.url), new Uint8Array(await response.arrayBuffer()));
  return localCoverUrl;
}

async function generateSeason(config, aniListIndex, resolveIdentity, syoboiSchedules) {
  const cards = parseCards(await fetchHtml(config.url), config);
  const anime = [];
  for (const [index, card] of cards.entries()) {
    const enriched = enrichYucRecord(card, index, config.url, findMatch(card, aniListIndex));
    const identity = resolveIdentity(card, config.url, enriched.id);
    const record = { ...enriched, ...identity };
    anime.push({
      ...applySyoboiSchedule(record, syoboiSchedules.get(record.id)),
      coverUrl: await downloadCover(card.coverUrl, identity.coverUrl),
    });
  }
...
async function syoboiSchedulesForYear(year) {
  try {
    const snapshot = await import(new URL(`../data/syoboi-history-${year}.js`, import.meta.url));
    return new Map((snapshot[`syoboiHistory${year}`]?.entries ?? []).map((entry) => [entry.recordId, entry]));
  } catch (error) {
    if (error.code === "ERR_MODULE_NOT_FOUND") return new Map();
    throw error;
  }
}
...
async function main() {
  ...
  const [aniListCatalogs, syoboiSchedules, { allAnime }] = await Promise.all([
    import(new URL(`../data/anilist-${year}.js`, import.meta.url)),
    syoboiSchedulesForYear(year),
    import(new URL("../data/anime.js", import.meta.url)),
  ]);
  ...
  const resolveIdentity = createCatalogIdentityResolver(allAnime);
  const catalogs = [];
  for (const config of yucSeasons) {
    catalogs.push(await generateSeason(config, aniListIndex, resolveIdentity, syoboiSchedules));
  }
  resolveIdentity.assertComplete(yucSeasons.map(({ url }) => url));
  ...
  const target = new URL(`../data/yuc-history-${year}.js`, import.meta.url);
  const staged = new URL(`../data/.yuc-history-${randomUUID()}.tmp`, import.meta.url);
  try {
