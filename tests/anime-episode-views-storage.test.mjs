...
test("saves a watched range through one request and one atomic SQLite transaction", async () => {
  const [route, page] = await Promise.all([
    readFile(new URL("../app/api/anime-episode-views/route.ts", import.meta.url), "utf8"),
    readFile(new URL("../app/hooks/use-viewer.tsx", import.meta.url), "utf8"),
  ]);
  ...
  assert.match(putRoute, /db\.transaction\(/);
  assert.match(putRoute, /watchedEpisodes\.map/);
  assert.match(putRoute, /values\(\s*watchedEpisodes\.map/);
  assert.match(putRoute, /return privateJson\(\{ watchedEpisodes, watched \}\)/);

  assert.match(
    toggleEpisodeView,
    /JSON\.stringify\(\{ watchedEpisodes: episodeViews, watched \}\)/,
  );
  assert.match(toggleEpisodeView, /const watched = !isEpisodeViewWatched\(previous, watchedEpisode\)/);
