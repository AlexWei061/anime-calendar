...
test("ships YUC historical catalogs with Chinese titles, covers, and AniList broadcast details", async () => {
  ...
  assert.deepEqual(
    seasons.slice(0, 6).map(({ timelineStartHour }) => timelineStartHour),
    [5, 5, 5, 5, 5, 5],
  );
  assert.deepEqual(
    seasons.slice(0, 6).map(({ sourceName, sourceUrl }) => ({ sourceName, sourceUrl })),
    [
  ...
  assert.ok(
    historicalAnime.every(
      ({ titleZh, coverUrl }) =>
        ...
        /^\/covers\/yuc\/history-202[56]-/.test(coverUrl),
    ),
  );
  await Promise.all(
...
test("keeps the historical pilot as generated local data", async () => {
  const [generated2025, generated2026] = await Promise.all([
  ...
  for (const seasonName of ["winter2025", "spring2025", "summer2025", "fall2025", "winter2026", "spring2026"]) {
    assert.match(seasonName.endsWith("2025") ? generated2025 : generated2026, new RegExp(`export const ${seasonName}`));
  }
});
